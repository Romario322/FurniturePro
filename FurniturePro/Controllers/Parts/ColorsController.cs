...
public class ColorsController : BaseController<int, ColorDto, CreateColorDto, UpdateColorDto>
{
    ...
    public override Task<ActionResult<List<ColorDto>>> GetAll(CancellationToken ct = default) => base.GetAll(ct);

    [HttpGet("sync")]
    [Authorize(Roles = nameof(SystemRoleEnum.Administrator) + "," + nameof(SystemRoleEnum.Constructor) + "," + nameof(SystemRoleEnum.SalesManager) + "," + nameof(SystemRoleEnum.WorkshopMaster))]
    public override Task<ActionResult<List<ColorDto>>> Sync([FromQuery] DateTime since, CancellationToken ct = default) => base.Sync(since, ct);
}