...
namespace FurniturePro.WebAdmin.Pages.API
{
    public class ClientsModel : PageModel
    {
        ...
        public async Task<JsonResult> OnGetAsync(string dateTime, CancellationToken ct)
        {
            ...
            try
            {
                var itemsUrl = $"api/clients/after {dateTime}";
                var deletedUrl = $"api/deletedIds/after {dateTime} clients";
                ...
                if (itemsResponse.IsSuccessStatusCode)
                {
                    items = await itemsResponse.Content
                        .ReadFromJsonAsync<List<ClientDTO>>(cancellationToken: ct) ?? new();
                }
