...
namespace FurniturePro.WebAdmin.Pages.API
{
    public class OrdersModel : PageModel
    {
        ...
        public OrdersModel(IOrderService orderService, IDeletedIdService deletedIdService)
        {
        ...
        public async Task<JsonResult> OnGetAsync(string dateTime, CancellationToken ct)
        {
            ...
            try
            {
                items = await _orderService.GetAfterDateAsync(dateTime, ct);
                deletedItems = await _deletedIdService.GetAfterDateAsync(dateTime, "orders", ct);
            }
            catch (Exception ex)
            {
