...
namespace FurniturePro.WebAdmin.Pages.API
{
    public class StatusesModel : PageModel
    {
        ...
        public StatusesModel(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<JsonResult> OnGetAsync(string dateTime, CancellationToken ct)
        {
            ...
            try
            {
                var itemsUrl = $"api/statuses/after {dateTime}";
                var deletedUrl = $"api/deletedIds/after {dateTime} statuses";
                ...
                if (itemsResponse.IsSuccessStatusCode)
                {
                    items = await itemsResponse.Content
                        .ReadFromJsonAsync<List<StatusDTO>>(cancellationToken: ct) ?? new();
                }
