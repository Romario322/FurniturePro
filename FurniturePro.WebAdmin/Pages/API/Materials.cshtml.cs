...
namespace FurniturePro.WebAdmin.Pages.API
{
    public class MaterialsModel : PageModel
    {
        ...
        public async Task<JsonResult> OnGetAsync(string dateTime, CancellationToken ct)
        {
            ...
            try
            {
                var itemsUrl = $"api/materials/after {dateTime}";
                var deletedUrl = $"api/deletedIds/after {dateTime} materials";
                ...
                if (itemsResponse.IsSuccessStatusCode)
                {
                    items = await itemsResponse.Content
                        .ReadFromJsonAsync<List<MaterialDTO>>(cancellationToken: ct) ?? new();
                }
